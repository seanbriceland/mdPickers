...
function TimePickerCtrl($scope, $mdDialog, time, autoSwitch, $mdMedia) {
    ...
    /**
     * PointMan updates:
     * - Instead of always switching view on selection of hour or minute, ONLY switch view when hour is selected.
     */
    this.switchView = function () {
        if (self.currentView === self.VIEW_HOURS) self.currentView = self.VIEW_MINUTES;
    };
...
function ClockCtrl($scope) {
    ...
    this.init = function () {
        ...
        switch (self.type) {
            case TYPE_HOURS:
                for (var i = 1; i <= 12; i++) self.steps.push(i);
                self.selected = self.time.hours() || 0;
                if (self.selected > 12) self.selected -= 12;
                break;
            case TYPE_MINUTES:
                for (var i = 5; i <= 55; i += 5) self.steps.push(i);
                self.steps.push(0);
    ...
    return {
        ...
        template: function (element, attrs) {
            var noFloat = angular.isDefined(attrs.mdpNoFloat),
                ...
                openOnClick = angular.isDefined(attrs.mdpOpenOnClick);
